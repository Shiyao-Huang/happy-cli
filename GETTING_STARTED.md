...
monitor.on('usage', (usage) => {
  console.log(
    `New request: ${usage.totalTokens} tokens, $${usage.costUSD.toFixed(6)}`
  );
});
...
1. **Use monitored queries by default**: Replace `query()` with
   `createMonitoredQuery()` to get automatic monitoring
...
- Set up [automated switching](#use-case-2-auto-switch-based-on-budget) based on
  your budget
