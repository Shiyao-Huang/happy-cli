...
async function readSessionData(options: any): Promise<any> {
  ...
  if (!existsSync(projectsDir)) {
    console.log(
      chalk.yellow('No Claude projects found. Make sure you have run Claude Code first.')
    );
    return [];
  }
...
function displayStats(stats: any[], timeRange: string, options: any): void {
  ...
  // Table format
  console.log(
    chalk.bold('Date         Input Tokens    Output Tokens   Total Tokens    Cost       Models')
  );
  console.log('-'.repeat(80));

  for (const stat of stats) {
    const modelStr =
      stat.modelsUsed.length > 2
