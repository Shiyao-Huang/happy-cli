...
export async function handleTokenStats(options: TokenStatsOptions = {}) {
  ...
  const displayStats = () => {
    const stats = options.model ? monitor.getUsageByModel(options.model) : monitor.getStats();
  ...
  if (activeProfile && options.format !== 'compact') {
    ...
    console.log(
      `  Cost: $${activeProfile.costPer1KInput}/1K input, $${activeProfile.costPer1KOutput}/1K output\n`
    );
  }
