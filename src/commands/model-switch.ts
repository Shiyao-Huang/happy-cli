...
async function listModels(modelManager: any, format?: string) {
  ...
  profiles.forEach((profile: ModelProfile) => {
    ...
    console.log(
      `   Cost: $${profile.costPer1KInput}/1K input, $${profile.costPer1KOutput}/1K output`
    );
...
async function addModel(modelManager: any, name: string, options: ModelSwitchOptions) {
  ...
  const avgUsage =
    stats.length > 0
      ? {
          avgInputTokens:
            stats.reduce((s: any, st: any) => s + st.totalInputTokens, 0) / stats.length,
          avgOutputTokens:
            stats.reduce((s: any, st: any) => s + st.totalOutputTokens, 0) / stats.length,
...
async function autoSwitch(modelManager: any, pattern: 'expensive' | 'cheap' | 'balanced') {
  ...
  if (success) {
    ...
    console.log(
      chalk.green(
        `✓ Auto-switched to "${active?.displayName || active?.name}" based on ${pattern} usage pattern`
      )
    );
  } else {
