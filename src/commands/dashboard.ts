...
export async function handleDashboard(options: DashboardOptions = {}) {
  ...
  console.log(
    chalk.blue.bold(
      '║                     Claude Code Token Monitor Dashboard                      ║'
    )
  );
  console.log(
    chalk.blue.bold(
  ...
  const update = () => {
    ...
    // Active model
    console.log(
      chalk.bold('┌─ ') + chalk.green('Active Model') + chalk.bold(' ─' + '─'.repeat(60) + '┐')
    );
    if (activeModel) {
      const cost = `$${activeModel.costPer1KInput}/1K in, $${activeModel.costPer1KOutput}/1K out`;
      console.log(
        `│ ${chalk.cyan(activeModel.displayName || activeModel.name)} (${activeModel.modelId})`
      );
    ...
    console.log(
      chalk.bold('┌─ ') + chalk.green('Current Session') + chalk.bold(' ─' + '─'.repeat(58) + '┐')
    );
    ...
    console.log(
      `│ ${chalk.bold('  Input:')}        ${chalk.cyan(stats.totalInput.toLocaleString())}`
    );
    console.log(
      `│ ${chalk.bold('  Output:')}       ${chalk.cyan(stats.totalOutput.toLocaleString())}`
    );
    console.log(
      `│ ${chalk.bold('Total Cost:')}    ${chalk.yellow('$' + stats.totalCost.toFixed(6))}`
    );
    console.log(`│ ${chalk.bold('Requests:')}      ${chalk.green(stats.requestCount)}`);
    console.log(
      `│ ${chalk.bold('Duration:')}      ${chalk.blue(sessionDuration.toFixed(1) + 's')}`
    );
    console.log('└' + '─'.repeat(74) + '┘\n');

    // Real-time rate
    console.log(
      chalk.bold('┌─ ') + chalk.green('Real-time Rate') + chalk.bold(' ─' + '─'.repeat(60) + '┐')
    );
    const rateAnimation = animationFrames[animationFrame % animationFrames.length];

    console.log(
      `│ ${chalk.bold('Tokens/sec:')}     ${chalk.magenta(stats.currentRate.tokensPerSecond.toFixed(2))} ${rateAnimation}`
    );
    console.log(
      `│ ${chalk.bold('Cost/sec:')}      ${chalk.magenta('$' + stats.currentRate.costPerSecond.toFixed(6))}`
    );

    // Rate bar visualization
    const barWidth = 60;
    const maxRate = Math.max(
      stats.currentRate.tokensPerSecond,
      stats.averageRate.tokensPerSecond,
      1
    );
    const currentBarLength = Math.min(
      barWidth,
      (stats.currentRate.tokensPerSecond / maxRate) * barWidth
    );
    const averageBarLength = Math.min(
      barWidth,
      (stats.averageRate.tokensPerSecond / maxRate) * barWidth
    );

    console.log(`│`);
    console.log(
      `│ ${chalk.bold('Current:')}  ${chalk.magenta('█'.repeat(Math.floor(currentBarLength)) + '░'.repeat(barWidth - Math.floor(currentBarLength)))}`
    );
    console.log(
      `│ ${chalk.bold('Average:')} ${chalk.blue('█'.repeat(Math.floor(averageBarLength)) + '░'.repeat(barWidth - Math.floor(averageBarLength)))}`
    );
    console.log('└' + '─'.repeat(74) + '┘\n');
    ...
    if (topModels.length > 0) {
      console.log(
        chalk.bold('┌─ ') + chalk.green('Top Models') + chalk.bold(' ─' + '─'.repeat(63) + '┐')
      );
      topModels.forEach((model, index) => {
        const rank = ['', '①', '②', '③'][index] || '④';
        console.log(
          `│ ${rank} ${chalk.cyan(model.model)} - ${chalk.green(model.requestCount)} req, ${chalk.yellow('$' + model.totalCost.toFixed(4))}`
        );
      });
      console.log('└' + '─'.repeat(74) + '┘\n');
    }
    ...
    if (recent.length > 0) {
      console.log(
        chalk.bold('┌─ ') + chalk.green('Recent Usage') + chalk.bold(' ─' + '─'.repeat(62) + '┐')
      );
      recent
        .slice(-5)
        .reverse()
        .forEach((usage) => {
          ...
          console.log(
            `│ ${chalk.gray(time)}  ${tokens.toLocaleString()} tokens  ${chalk.yellow(cost)}  ${chalk.cyan(usage.model || 'unknown')}`
          );
        });
      console.log('└' + '─'.repeat(74) + '┘\n');
    }
    ...
    console.log(
      chalk.gray(
        `Last updated: ${new Date().toLocaleTimeString()} | Session: ${(elapsed / 1000).toFixed(0)}s`
      )
    );
...
export async function handleSimpleDashboard() {
  ...
  const update = () => {
    ...
    console.log(
      `Current Rate:  ${chalk.magenta(stats.currentRate.tokensPerSecond.toFixed(2) + ' tokens/sec')}`
    );
    console.log(
      `Average Rate:  ${chalk.blue(stats.averageRate.tokensPerSecond.toFixed(2) + ' tokens/sec')}\n`
    );
