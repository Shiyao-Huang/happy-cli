...
describe.skipIf(!(await isServerHealthy()))('Daemon Integration Tests', { timeout: 20_000 }, () => {
  ...
  it('should track both daemon-spawned and terminal sessions', async () => {
    ...
    const terminalHappyProcess = spawnHappyCLI(
      ['--happy-starting-mode', 'remote', '--started-by', 'terminal'],
      {
        cwd: '/tmp',
        detached: true,
        stdio: 'ignore',
    ...
    const daemonSession = sessions.find((s: any) => s.happySessionId === spawnResponse.sessionId);
    ...
    async () => {
      ...
      try {
        ...
        await new Promise((resolve) =>
          setTimeout(
            resolve,
            parseInt(process.env.HAPPY_DAEMON_HEARTBEAT_INTERVAL || '30000') + 10_000
          )
        );
