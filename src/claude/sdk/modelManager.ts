...
export class ModelManager {
              ...
              for (const jsonStr of jsonMatches) {
                try {
                  ...
                  if (config && config.env) {
                    ...
                    // Add GLM model
                    if (
                      env.ANTHROPIC_BASE_URL?.includes('bigmodel.cn') &&
                    ...
                    // Add Kimi model
                    if (
                      env.ANTHROPIC_BASE_URL?.includes('moonshot.cn') &&
  ...
  upsertProfile(
    profile: Omit<ModelProfile, 'createdAt' | 'updatedAt'> &
      Partial<Pick<ModelProfile, 'createdAt' | 'updatedAt'>>
  ): ModelProfile {
  ...
  removeProfile(name: string): boolean {
    ...
    if (removed) {
      if (wasActive) {
        this.activeProfile = this.profiles.size > 0 ? Array.from(this.profiles.keys())[0] : null;
      }
      this.saveConfig();
    }
      ...
      if (currentUsage) {
        const estimatedCost =
          (currentUsage.avgInputTokens * profile.costPer1KInput +
            currentUsage.avgOutputTokens * profile.costPer1KOutput) /
          1000;
